...
const KEY = "BuildProcessing";

export default function loadBuildProcessing() {
    const sandboxEnabled = Sandbox.getEnabled();
    const updatePerModel = new Map<Model, () => void>();
    const cleanupPerModel = new Map<Model, () => void>();
    let area: Area | undefined;

    // Continuously load models for placed items in the workspace
    RunService.BindToRenderStep(KEY, 1, () => {
    ...
    function onModelAdded(model: Instance) {
        if (!model.IsA("Model") || updatePerModel.has(model)) return;
        ...
        connections.add(model.GetAttributeChangedSignal("Dragging").Connect(update));
        connections.add(model.GetAttributeChangedSignal("Hovering").Connect(update));
        connections.add(
            model.Destroying.Once(() => {
                updatePerModel.delete(model);
                cleanupPerModel.delete(model);
            }),
        );
        updatePerModel.set(model, update);
        cleanupPerModel.set(model, () => {
    ...
    const buildConnection = Workspace.GetAttributeChangedSignal("BuildMode").Connect(() => {
        for (const [, update] of updatePerModel) {
            update();
        }
    ...
    const interactiveConnection = CollectionService.GetInstanceAddedSignal("Interactive").Connect((interactive) => {
        (interactive as Interactive).Enabled = !Workspace.GetAttribute("BuildMode") as boolean;
    });
    ...
    return {
        destroy: () => {
            ...
            interactiveConnection.Disconnect();
            updatePerModel.clear();
            for (const [, cleanup] of cleanupPerModel) {
                cleanup();
            }
        },
    };
