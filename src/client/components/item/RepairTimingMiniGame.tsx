...
export default function RepairTimingMiniGame({
    ...
    useEffect(() => {
        if (phase === "success" && resultTier !== undefined) {
            ...
            const score = math.clamp(1 - math.abs(currentProgressRef.current) / HOT_WINDOW, 0, 1);
            onSuccess?.(score, tier);
        }
    }, [phase]);
                ...
                <frame BackgroundTransparency={1} Size={new UDim2(1, 0, 1, 0)}>
                    ...
                    >
                        ...
                        <frame
                            ...
                            BackgroundTransparency={1}
                            Position={new UDim2(0.5, 0, 0.5, 0)}
                            Size={new UDim2(1, math.abs(approachProgress) * 100, 1, math.abs(approachProgress) * 100)}
                        >
                            <uicorner CornerRadius={new UDim(1, 0)} />
                            <uistroke
...
function ResultTierDisplay({ tier }: { tier: ResultTier }) {
...
function SparkEffect({
    ...
    return (
        <imagelabel
            ...
            ImageColor3={Color3.fromRGB(255, 223, 62)}
