...
export = function () {
    ...
    describe("Verbose Droplet Value Calculation", () => {
        it("tracks base droplet value without any modifiers", () => {
            ...
            expect(result.factors).to.be.ok();
            expect(result.factors.size()).to.equal(0);

            // Should have base value set
            const baseValue = result.baseValue;
            expect(baseValue).to.be.ok();
            expect(baseValue.equals(Droplet.TheFirstDroplet.value)).to.equal(true);
        ...
        it("tracks upgrader boosts in verbose factors", () => {
            ...
            expect(upgraderFactors.size()).to.equal(1);

            const [label, operative] = upgraderFactors[0];
            expect(operative.add).to.be.ok();
            const upgraderAdd = TheFirstUpgrader.findTrait("Upgrader")?.add?.get("Funds");
            expect(upgraderAdd).to.be.ok();
            expect(operative.add?.get("Funds")?.equals(upgraderAdd!)).to.equal(true);
        ...
        it("tracks multiple upgrader boosts separately", () => {
            ...
            expect(result.factors.size() > 1).to.equal(true);
            ...
            expect(additiveFactors.size()).to.equal(1);
            expect(multiplicativeFactors.size()).to.equal(1);

            expect(additiveFactors[0][1].add).to.be.ok();
            expect(multiplicativeFactors[0][1].mul).to.be.ok();
        ...
        it("does not track health nerf when droplet health is at 100", () => {
            ...
            const healthFactors = result.factors.filter(([label]) => label === "HEALTH");
            expect(healthFactors.size()).to.equal(0);
        ...
        it("tracks lightning surge boost", () => {
            ...
            expect(lightningFactors.size()).to.equal(1);
            ...
            expect(factor.mul.equals(new OnoeNum(10))).to.equal(true);
        ...
        it("tracks weather boost when enabled", () => {
            ...
            expect(weatherFactors.size()).to.equal(1);
            ...
            expect(factor.mul.equals(new OnoeNum(2.5))).to.equal(true);
        ...
        it("tracks sky droplet nerf", () => {
            ...
            expect(skyFactors.size()).to.equal(1);

            const [, factor] = skyFactors[0];
            expect(factor.mul).to.be.ok();
            expect(factor.inverse).to.equal(true);
            if (factor.mul === undefined) throw "factor.mul is undefined";
            if (!("mantissa" in factor.mul)) throw "factor.mul should be an OnoeNum";

            expect(factor.mul.equals(new OnoeNum(250))).to.equal(true);
        ...
        it("tracks softcap application with very high values", () => {
            ...
            expect(softcapDivFactors.size() > 0).to.equal(true);
            expect(softcapRootFactors.size() > 0).to.equal(true);

            // Verify inverse flag is set
            expect(softcapDivFactors[0][1].inverse).to.equal(true);
            expect(softcapRootFactors[0][1].inverse).to.equal(true);
        ...
        it("tracks all factors in correct order through full calculation pipeline", () => {
            ...
            expect(result.factors.size() > 0).to.equal(true);
            ...
            expect(coalesced).to.be.ok();

            // The final value should incorporate all factors
            const fundsResult = coalesced.get("Funds");
            expect(fundsResult).to.be.ok();
            expect(fundsResult!.moreThan(new OnoeNum(0))).to.equal(true);
        ...
        it("maintains factor order consistency across multiple calculations", () => {
            ...
            expect(result1.factors.size()).to.equal(result2.factors.size());
        ...
        it("does not include weather factor when weather boost is disabled", () => {
            ...
            const weatherFactors = result.factors.filter(([label]) => label === "WEATHER");
            expect(weatherFactors.size()).to.equal(0);
        ...
        it("coalesce returns correct value matching manual calculation", () => {
            ...
            expect(coalescedFunds.equals(expected)).to.equal(true);
