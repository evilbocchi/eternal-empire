...
import { AREAS, emitEffect, getNPCModel, getSound, getWaypoint } from "shared/constants";
...
const unlockWall = () => {
    ...
    const keyModel = ReserveModels.fetchReserve(IrregularlyShapedKey.id);
...
export = new Quest(script.Name)
    ...
    .addStage(new Stage()
        .setDescription("Ask the %npc% about the key.")
        .setNPC("Old Noob", true)
    ...
    .addStage(new Stage()
        .setDescription(`Give the ${ExcavationStone.name} back to the %npc%.`)
        .setNPC("Old Noob", true)
    ...
    .addStage(new Stage()
        .setDescription(`Follow the %npc% to use the key.`)
