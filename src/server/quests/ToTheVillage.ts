...
import ChargedEmpoweredBrick from "shared/items/miscellaneous/ChargedEmpoweredBrick";
...
export = new Quest(script.Name)
    ...
    .addStage(new Stage()
        ...
        .onStart((stage) => {
            const continuation = new Dialogue(SlamoRefugee, "Thank you so much! I feel the power of the brick already!")
                ...
                .root;
            const connection = GameUtils.dialogueFinished.connect((dialogue) => {
                if (dialogue === stage.dialogue && GameUtils.itemsService.getItemAmount(EmpoweredBrick.id) >= 1) {
                    GameUtils.talk(continuation);
                }
                if (dialogue === continuation && GameUtils.takeQuestItem(EmpoweredBrick.id, 1)) {
                    stage.completed.fire();
                }
            });
            return () => connection.disconnect();
    ...
    .addStage(new Stage()
        ...
        .onStart((stage) => {
            ...
            const connection = GameUtils.dialogueFinished.connect((dialogue) => {
                if (dialogue === stage.dialogue && GameUtils.itemsService.getItemAmount("XLWool") >= 2) {
                    GameUtils.talk(continuation);
                }
                if (dialogue === continuation && GameUtils.takeQuestItem("XLWool", 2)) {
                    stage.completed.fire();
                }
            });
            return () => connection.disconnect();
    ...
    .addStage(new Stage()
        ...
        .onStart((stage) => {
            ...
            task.spawn(() => {
                ...
                GameUtils.addDialogue(continuation, 4);
            });
        ...
        .onStart((stage) => {
            ...
            const connection = GameUtils.dialogueFinished.connect((dialogue) => {
                if (dialogue === continuation) {
                    ...
                    task.wait(1);
                    GameUtils.talk(continuation2);
                }
                else if (dialogue === continuation2) {
                    stage.completed.fire();
                    GameUtils.giveQuestItem(ChargedEmpoweredBrick.id, 1);
    ...
    .addStage(new Stage()
        ...
        .onStart((stage) => {
            ...
            const connection = GameUtils.itemsService.itemsPlaced.connect((_, placedItems) => {
                let placed = false;
                ...
                GameUtils.unlockedAreasService.unlockArea("SlamoVillage");
                stage.completed.fire();
            });
    ...
    .addStage(new Stage()
        ...
        .onStart((stage) => {
            GameUtils.setEventCompleted("ImprisonedSlamoRefugee", true);
    ...
    .onInit(() => {
        ...
        GameUtils.addCompletionListener("ImprisonedSlamoRefugee", () => {
            const refugee = getNPCModel("Slamo Refugee");
            refugee.FindFirstChild("FishingRod")?.Destroy();
            const humanoid = refugee.WaitForChild("Humanoid") as Humanoid;
            humanoid.RootPart!.Anchored = true;
