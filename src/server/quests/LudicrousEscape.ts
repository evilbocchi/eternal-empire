...
import { AREAS, emitEffect, getNPCModel, getSound, getWaypoint, PLACED_ITEMS_FOLDER, RESET_LAYERS } from "shared/constants";
...
export = new Quest(script.Name)
    ...
    .addStage(new Stage()
        .setDescription(`Confront %npc% about his past.`)
        .setNPC("Simpul")
        .setDialogue(
    ...
    .addStage(new Stage()
        .setDescription(`Help the police find and re-capture %npc%.`)
        .setNPC("Simpul")
        .setDialogue(
        ...
        .onStart((stage) => {
            ...
            const start = new Dialogue(SlamoReceptionist, "So, about that explosion above... Simpul escaped, didn't he.")
                ...
                .monologue("You'll need some stuff for the lure. Get 5 Wood and 3 Rock from the store and head back to me. I'll put some stuff together based on some model Slamos.")
                ...
                .root;
            const checking = new Dialogue(SlamoReceptionist, "Do you have the 5 Wood and 3 Rock?");
    ...
    .addStage(new Stage()
        .setDescription(`Lure %npc% and trap him.`)
        .setFocus(ring)
        .setNPC("Simpul")
        .setDialogue(
        ...
        .onStart((stage) => {
            ...
            let initiated = false;
            const update = (placedItem: PlacedItem) => {
                statue = PLACED_ITEMS_FOLDER.FindFirstChild(placedItem.placementId) as Model | undefined;
                task.wait(0.5);
            ...
            const placedItems = GameUtils.empireData.items.placed;
            ...
            const connection1 = GameUtils.itemsService.itemPlaced.connect((_player, placedItem) => {
                if (placedItem.item === SlamoStatue.id)
                    update(placedItem);
            });
            const connection2 = GameUtils.dialogueFinished.connect((dialogue) => {
                if (dialogue === poured) {
                    const model = ReserveModels.fetchReserve(WinsomeBucket.id);
                    if (model === undefined)
                        return;
    ...
    .addStage(new Stage()
        .setDescription(`Serve justice to %npc%.`)
        .setNPC("Simpul")
        .onStart((stage) => {
        ...
        GameUtils.dialogueFinished.connect((dialogue) => {
            if (dialogue === Andy.defaultDialogue) {
                ...
                if (rng === 1) {
                    ...
                    CurrencyService.incrementCost("Funds Bombs", new OnoeNum(1));
                }
                else if (rng === 2) {
                    GameUtils.talk(new Dialogue(Andy, "As your reward, I gave you a bit of my Skill. Hopefully, it'll help you out!"));
                    CurrencyService.incrementCurrencies(GameUtils.resetService.getResetReward(RESET_LAYERS.Skillification).div(5).costPerCurrency);
                }
                else if (rng === 3) {
