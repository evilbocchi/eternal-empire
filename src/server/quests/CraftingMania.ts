...
import { AREAS, getNPCModel, getWaypoint } from "shared/constants";
...
import Shopkeeper1 from "shared/npcs/Shopkeeper 1";
...
export = new Quest(script.Name)
    ...
    .addStage(new Stage()
        .setDescription(`Collect 1 ${Wool.id} and 3 ${Grass.id}. Ask Ricarg how to collect the grass.`)
        .setNPC("Ricarg", true)
        .setDialogue(
    ...
    .onInit((utils) => {
        GameUtils.dialogueFinished.connect((dialogue) => {
            if (dialogue === Shopkeeper1.defaultDialogue) {
                GameUtils.openShop(Wool.id);
            }
        });
    })
