...
import { CHALLENGES, CHALLENGE_UPGRADES, REWARD_UPGRADES } from "server/Challenges";
...
import { ASSETS, CHALLENGE_OPTIONS, CURRENT_CHALLENGE_WINDOW, RESET_LAYERS } from "shared/constants";
import Item from "shared/item/Item";
import Packets from "shared/network/Packets";
import StringBuilder from "shared/utils/StringBuilder";
import RomanNumerals from "shared/utils/vrldk/RomanNumerals";
...
export class ChallengeService implements OnInit {
    ...
    startChallenge(player: Player, challengeId: ChallengeId) {
        ...
        if (challengeDetails.resets !== undefined) {
            ...
            if (resetLayer !== undefined) {
                ...
                data.backup.upgrades = new Map();
                for (const [upgrade, amount] of data.upgrades)
                    if (resetLayer.resettingUpgrades.includes(upgrade))
                        data.backup.upgrades.set(upgrade, amount);

                data.backup.currencies = new Map();
                for (const [currency, amount] of data.currencies)
                    if (resetLayer.resettingCurrencies.includes(currency))
                        data.backup.currencies.set(currency, amount);

                this.resetService.performReset(resetLayer);
            }
        }
        data.currentChallengeStartTime = data.playtime;
    ...
    getTitleLabel(challenge: ChallengeDetails, id?: string, level?: number) {
        ...
        return `${challenge.name} ${(RomanNumerals.ToRomanNumerals(level))}`;
    }
    ...
    refreshCurrentChallenge() {
        ...
        if (challengeId === undefined) {
            CURRENT_CHALLENGE_WINDOW.Visible = false;
            CHALLENGE_OPTIONS!.Visible = true;
            ...
            CURRENT_CHALLENGE_WINDOW.Visible = true;
            CHALLENGE_OPTIONS!.Visible = false;
            const challenge = CHALLENGES[challengeId];
            const requirement = "Requirement: " + this.getRequirementLabel(challenge);
            CURRENT_CHALLENGE_WINDOW.RequirementLabel.Text = requirement;
            const currentLevel = this.getChallengeLevel(challengeId);
            const title = this.getTitleLabel(challenge, challengeId, currentLevel);
            CURRENT_CHALLENGE_WINDOW.TitleLabel.Text = title;
            CURRENT_CHALLENGE_WINDOW.TitleLabel.UIGradient.Color = challenge.color;
        ...
        if (data.backup.currencies !== undefined) {
            for (const [currency, amount] of data.backup.currencies) {
                ...
                if (currentAmount === undefined || new OnoeNum(currentAmount).lessThan(amount))
                    this.currencyService.setCost(currency, new OnoeNum(amount));
            }
    ...
    challengeEffect() {
        ...
        const t = tick();
        if (challengeCompleted === true && t - this.lastCompletion > 2) {
    ...
    refreshChallenges() {
        let i = 0;
        CHALLENGE_OPTIONS?.GetChildren().forEach((instance) => {
        ...
        for (const [key, challenge] of pairs(CHALLENGES)) {
            ...
            challengeOption.Parent = CHALLENGE_OPTIONS;
            if (currentLevel > 1) {
