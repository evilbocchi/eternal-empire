...
import { ASSETS, AREAS, getSound } from "shared/constants";
...
namespace Challenges {
    ...
    export const MeltingEconomy = {
        ...
        cap: 5,
        challengeUpgrade: new GainUpgrade().setPow(x => new Price().setCost("Funds", meltingEconomyNerf(x))),
        rewardUpgrade: new GainUpgrade().setMul(x => new Price().setCost("Funds", new OnoeNum(1.75).pow(x))),
    ...
    const spawnMeteor = () => {
        ...
        const grid = AREAS.BarrenIslands.grid!;
    ...
    export const CataclysmicWorld = {
        ...
        resets: "Skillification",
        rewardUpgrade: new GainUpgrade().setMul(x => new Price().setCost("Skill", new OnoeNum(1.5).pow(x))),
        goal: [Admiration, Codependence],
        order: 2,
    }
    ...
    export const PinnedProgress = {
        ...
        challengeUpgrade: new GainUpgrade().setPow(x => {
            ...
            return new Price().setCost("Funds", res).setCost("Power", res);
        }),
        restrictItems: (item: Item) => {
            const types = item.types;
            let count = 0;
            for (const t of types) {
                if (t === "Conveyor" || t === "Operative") {
                    ++count;
                }
                else if (t === "Upgrader") {
                    const u = (item as Upgrader);
                    if (u.add !== undefined || u.mul !== undefined || u.pow !== undefined)
                        return false;
                }
                else {
                    return false;
        ...
        rewardUpgrade: new GainUpgrade().setMul(x => new Price().setCost("Power", new OnoeNum(2).pow(x))),
        resets: "Skillification",
        goal: [Admiration, Codependence],
        order: 3,
    }
