...
const limit = new InfiniteMath([4, 12]);

export = new Upgrader("ShockingRefiner")
.setName("Shocking Refiner")
.setDescription("Gives droplets a bigger shock than an Energised Refiner. Funds boost increases with Power, maxing out at 4T W. Uses 2K W/s. <log13((2 * power) + 1) + 1>")
.setDifficulty(Difficulty.Exist)
.setPrice(new Price().setCost("Funds", new InfiniteMath([16.25, 15])).setCost("Power", 1760000), 1)
.setPrice(new Price().setCost("Funds", new InfiniteMath([24.6, 15])).setCost("Power", 3240000), 2)
