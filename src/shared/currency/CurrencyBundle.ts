...
class CurrencyBundle {
     ...
     */
    static currenciesToString(amountPerCurrency: Map<Currency, BaseOnoeNum>, isColored?: boolean, prefix?: string, suffix?: string) {
    ...
    equals(value: CurrencyBundle) {
        let equal = true;
        let otherSize = value.amountPerCurrency.size();
        let size = 0;
        for (const [currency, amount] of this.amountPerCurrency) {
