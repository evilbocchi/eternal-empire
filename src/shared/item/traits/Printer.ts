...
export default class Printer extends ItemTrait {
    static load(model: Model, printer: Printer) {
        ...
        loadEvent.Name = "Load";
        if (printer.area === undefined) error("No area");
    ...
    static clientLoad(model: Model, printer: Printer) {
        ...
        templateSetupOption.Heading.NameLabel.Text =
            "Setup " + (printedSetups === undefined ? "1" : printedSetups.size() + 1);
        ...
        const connection = Packets.printedSetups.observe((value) => {
            ...
            for (const setup of value) {
                if (setup.area !== printer.area || setupOptions === undefined) continue;
                ...
                if (cached !== undefined) {
                    ...
                    setupOption.Body.Autoload.ToggleButton.Activated.Connect(() =>
                        Packets.autoloadSetup.toServer(name),
                    );
                    setupOption.Parent = setupOptions;
                }

                const totalPrice = new CurrencyBundle();
                for (const [currency, amount] of setup.calculatedPrice) totalPrice.set(currency, new OnoeNum(amount));
            ...
            if (fill !== undefined) fill.Transparency = i > 0 ? 0.25 : 0.8;
        });
