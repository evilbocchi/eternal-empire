import { Debris, TweenService } from "@rbxts/services";
import { Dialogue } from "shared/NPC";
import Quest, { Stage } from "shared/Quest";
import { AREAS, ASSETS, getNPCModel, getSound, getWaypoint } from "shared/constants";
import FreddysUpgrader from "shared/items/miscellaneous/FreddysUpgrader";
import LostPendant from "shared/items/miscellaneous/LostPendant";
import Freddy from "shared/npcs/Freddy";

const waypoint2 = getWaypoint("AHelpingHand2");
const freddyModel = getNPCModel("Freddy");
const freddyHumanoid = freddyModel.FindFirstChildOfClass("Humanoid")!;
const freddyRootPart = freddyHumanoid.RootPart!;
const characterTween = TweenService.Create(freddyRootPart, new TweenInfo(1), { CFrame: waypoint2.CFrame });

const obstacleCourse = AREAS.BarrenIslands.map.WaitForChild("ObstacleCourse");
const ladder = obstacleCourse.WaitForChild("Ladder");
const ladderHandles = new Array<BasePart>();
for (let i = 1; i < 9; i++) {
    ladderHandles.push(ladder.FindFirstChild(i) as BasePart);
}
const strikeEffect = ASSETS.WaitForChild("StrikeEffect") as ParticleEmitter;

export = new Quest(script.Name)
.setName("A Helping Hand")
.setLength(1)
.setLevel(3)
.setOrder(6)
.addStage(new Stage()
    .setDescription(`Talk to %npc% at %coords%.`)
    .setNPC("Freddy", true)
    .setDialogue(
        new Dialogue(Freddy, "I hate this place... It's dreary, and nothing ever goes my way. Oh, hey.")
        .monologue("I didn't notice you there. I see you're quite rich. Maybe you can help me out with something.")
        .monologue("I've been stuck in this rut for too long, and I could really use a bit of your fortune to turn things around.")
        .monologue("What do you say? Care to lend a hand to someone down on their luck?")
        .root
    )
    .onStart((utils, stage) => {
        const continuation = new Dialogue(Freddy, "My name's Freddy. Follow me, I have something to show you.");
        freddyRootPart.Position = stage.position!;
        const connection = utils.dialogueFinished.connect((dialogue) => {
            if (dialogue === stage.dialogue) {
                utils.setEventCompleted("FreddyReveal", true);
                utils.talk(continuation);
            }              
            else if (dialogue === continuation)
                stage.completed.fire();
        });
        return () => connection.disconnect();
    })
)
.addStage(new Stage()
    .setDescription(`Follow %npc% to %coords%.`)
    .setNPC("Freddy")
    .setFocus(waypoint2)
    .setDialogue(
        new Dialogue(Freddy, "Follow me, I have something to show you.")
    )
    .onStart((utils, stage) => {
        utils.stopNPCAnimation(Freddy, "Default");
        task.wait(2);
        const connection = utils.leadToPoint(stage.npcHumanoid!, waypoint2.CFrame, () => stage.completed.fire());
        return () => connection.Disconnect();
    })
)
.addStage(new Stage()
    .setDescription(`Help %npc% out.`)
    .setNPC("Freddy")
    .setDialogue(
        new Dialogue(Freddy, "Rumors say that someone left a valuable item above these pillars for some reason. Weird, right? Who would leave something valuable up there?")
        .monologue("It sounds strange, but you never know - rumors often have a grain of truth to them.")
        .monologue("I've been thinking about it a lot, but I'm not Skilled enough to navigate the obstacles up there. The pillars are tricky, and it takes a certain level of agility and bravery to get through.")
        .monologue("Here's what we'll do: I'll make the ladder for you to climb up. It won't be easy, but I have faith in your abilities.")
        .monologue("Once you're up there, keep your eyes peeled for the item on top of the mountain formed from the pillars.")
        .monologue("I'll make the ladder for you. Get the item and come back to me once you're done.")
        .monologue("Good luck, and remember, I'm counting on you!")
        .root
    )
    .onStart((utils, stage) => {
        utils.stopNPCAnimation(Freddy, "Default");
        characterTween.Play();
        const connection = utils.dialogueFinished.connect((dialogue) => {
            if (stage.dialogue === dialogue)
                stage.completed.fire();
        });
        return () => connection.disconnect();
    })
)
.addStage(new Stage()
    .setDescription(`Complete the obstacle course and retrieve the item.`)
    .setNPC("Freddy")
    .setDialogue(
        new Dialogue(Freddy, "What are you waiting for? Go get it!")
    )
    .onStart((utils, stage) => {
        utils.stopNPCAnimation(Freddy, "Default");
        characterTween.Play();

        const hitSound = getSound("Hit");
        for (const handle of ladderHandles) {
            const strikeEffectClone = strikeEffect.Clone();
            strikeEffectClone.Parent = handle;
            strikeEffectClone.Emit(2);
            Debris.AddItem(strikeEffectClone);
            handle.Transparency = 0;
            handle.CanCollide = true;
            const hs = hitSound.Clone();
            hs.Parent = handle;
            hs.Play();
            task.wait(0.25);
        }
        utils.talk(new Dialogue(Freddy, "Off you go!"));
        task.wait(1);

        const connection = utils.onEventCompleted("AHelpingHandPendant", (isCompleted) => {
            if (isCompleted)
                stage.completed.fire();
        });
        return () => connection.disconnect();
    })
)
.addStage(new Stage()
    .setDescription(`Report back to %npc%.`)
    .setNPC("Freddy", true)
    .setDialogue(
        new Dialogue(Freddy, "I see you have the item. Nice work! I knew you could do it.")
        .monologue("To tell you the truth, my brother was the one who lost that item years ago. It holds a lot of sentimental value to me.")
        .monologue("Thanks for returning it back to me. As a token of my gratitude, I'll give you something else worth your time.")
        .monologue("Here's an item that'll boost your Funds gain by quite a bit. It's not just a reward, but a way for you to keep prospering in your adventures.")
        .monologue("Enjoy, and may it bring you as much fortune as you've brought me today. Thanks again!")
        .root
    )
    .onStart((utils, stage) => {
        utils.stopNPCAnimation(Freddy, "Default");
        characterTween.Play();
        const connection = utils.dialogueFinished.connect((dialogue) => {
            utils.takeQuestItem(LostPendant.id, 1);
            if (dialogue === stage.dialogue)
                stage.completed.fire();
        });
        return () => connection.disconnect();
    })
)
.onInit((utils) => {
    for (const handle of ladderHandles) {
        handle.Transparency = 1;
        handle.CanCollide = false;
    }
    strikeEffect.Enabled = false;
    utils.itemsBought.connect((_player, items) => {
        if (items.includes(LostPendant)) {
            utils.setEventCompleted("AHelpingHandPendant", true);
        }
    });
    utils.onEventCompleted("AHelpingHandPendant", (isCompleted) => {
        if (isCompleted)
            obstacleCourse.WaitForChild("LostPendant").Destroy();
    });
    utils.onEventCompleted("FreddyReveal", (isCompleted) => {
        if (isCompleted)
            freddyHumanoid.DisplayName = "";
    })
})
.setCompletionDialogue(
    new Dialogue(Freddy, "Thanks a lot for your work!")
)
.setReward({
    xp: 180,
    items: new Map([[FreddysUpgrader, 1]]),
});