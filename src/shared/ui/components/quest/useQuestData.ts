...
/**
 * Hook to get quest utility functions (these come from the original controller)
 */
export function useQuestHelpers() {
    const getLengthName = React.useCallback((length: number) => {
        switch (length) {
            case 0: return "Tiny";
            case 1: return "Short";
    ...
    const getLengthColor = React.useCallback((length: number) => {
        switch (length) {
    ...
    const getRewardLabel = React.useCallback((reward: Reward) => {
